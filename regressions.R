...
s1.data <- data.frame(1:25, s1, s1_upper, s1_lower)
s2.data <- data.frame(1:25, s2, s2_upper, s2_lower)
s3.data <- data.frame(1:25, s3, s3_upper, s3_lower)
s4.data <- data.frame(1:25, s4, s4_upper, s4_lower)
s5.data <- data.frame(1:25, s5, s5_upper, s5_lower)
s6.data <- data.frame(1:25, s6, s6_upper, s6_lower)
s7.data <- data.frame(1:25, s7, s7_upper, s7_lower)
s8.data <- data.frame(1:25, s8, s7_upper, s8_lower)
s9.data <- data.frame(1:25, s9, s7_upper, s9_lower)
s10.data <- data.frame(1:25, s10, s7_upper, s10_lower)
s11.data <- data.frame(1:25, s11, s7_upper, s11_lower)

s1_coef_plot <- ggplot(s1.data, aes(X1.25, s1))+
  geom_point()+
...
figure_random_effects_2 <- annotate_figure(figure_random_effects_2,
                                           ...
                                           fig.lab = "Figure 2", fig.lab.face = "bold")
## Redefine w/lag = 14 in order to compare across other regressions
reg6 <- plm(log(deaths_per_million) ~ lag(c1_school_closing, 14) + lag(c2_workplace_closing, 14 ) +
              lag(c3_cancel_public_events, 14) + lag(c4_restrictions_on_gatherings, 14) +
              lag(c5_close_public_transport, 14) + lag(c6_stay_at_home_requirements, 14) + 
              lag(c7_restrictions_on_internal_movement, 14) + lag(c8_international_travel_controls, 14) +
              lag(h1_public_information_campaigns, 14) + lag(h2_testing_policy, 14) + lag(h3_contact_tracing, 14) +
